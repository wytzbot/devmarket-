...
async function singleProduct(req, res, slug) {
  try {
...
async function listProducts(req, res, url) {
  try {
    ...
    if (search) { const q = encodeURIComponent(search); qs += `&or=(name.ilike.*${q}*,description.ilike.*${q}*)`; }
...
// GET /api/marketplace              -> list published products
// GET /api/marketplace?slug=<slug>  -> a single published product + its developer
export default async function handler(req, res) {
  if (!supabaseUrl || !serviceKey) return configError(res);
  const url = new URL(req.url, "http://localhost");
  const slug = req.query?.slug || url.searchParams.get("slug");
  if (slug) return singleProduct(req, res, String(slug).trim());
  return listProducts(req, res, url);
}
