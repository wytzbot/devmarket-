...
import { notify, mountToastHost, Select } from "./ui.jsx";
import "./styles.css";
mountToastHost();
...
function Layout({user,children}) {
  ...
  async function pushOn(){
    try{await enablePushNotifications();setPush(true)}catch(e){notify(e.message,"error")}
  }
  ...
  async function login(provider){
    try{ await signInWithPopup(auth,provider); nav("/dashboard") }
      ...
      if(["auth/popup-blocked","auth/popup-closed-by-user","auth/cancelled-popup-request","auth/operation-not-supported-in-this-environment"].includes(e.code)){
        try{ await signInWithRedirect(auth,provider); return } catch(e2){ notify(e2.message,"error"); return }
      }
...
function Explore(){
  const {products}=useProducts();
  const [q,setQ]=React.useState(""); const [cat,setCat]=React.useState("All"); const [sort,setSort]=React.useState("trending");
  const list=products.filter(p=>(cat==="All"||p.category===cat)&&(`${p.name} ${p.developer} ${p.tech.join(" ")}`.toLowerCase().includes(q.toLowerCase())))
    .sort((a,b)=>sort==="price_low"?a.price-b.price:sort==="price_high"?b.price-a.price:sort==="rating"?b.rating-a.rating:0);
  return <main className="page"><div className="eyebrow">MARKETPLACE</div><h1>Explore software</h1><p className="muted">Find production-ready building blocks without hunting through random repositories.</p>
    <div className="filterBar"><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search products, developers, technologies…"/><Select value={cat} onChange={setCat} options={["All","SaaS","AI","UI Kits"]}/><Select value={sort} onChange={setSort} options={[{value:"trending",label:"Trending"},{value:"new",label:"New"},{value:"rating",label:"Best rated"},{value:"price_low",label:"Price low to high"}]}/></div>
...
function Product(){
  ...
  async function buy(){
    if(!auth.currentUser){notify("Sign in first.","error");return}
    if(p.id.startsWith("demo-")){notify("This demo product is UI-only. Publish a real product to enable live checkout.");return}
...
function NewProduct({user}){
  ...
  const set=(e)=>{setF({...f,[e.target.name]:e.target.value}); if(fieldErrors[e.target.name])setFieldErrors({...fieldErrors,[e.target.name]:null})};
  function validate(){
    const errs={};
    if(!f.name.trim())errs.name="Name is required.";
    if(!f.slug.trim())errs.slug="Slug is required.";
    if(!f.description.trim())errs.description="Description is required.";
    if(!(Number(f.price)>0))errs.price="Enter a price greater than 0.";
    if(!/^\d+\.\d+\.\d+$/.test(f.version))errs.version="Use semantic version format, e.g. 1.0.0.";
    if(!file)errs.file="Choose a ZIP source archive.";
    return errs;
  }
  async function submit(e){
    e.preventDefault();
    const errs=validate();
    if(Object.keys(errs).length){setFieldErrors(errs);notify("Fix the highlighted fields before continuing.","error");return}
    if(!file?.name.toLowerCase().endsWith(".zip"))return notify("Choose a ZIP source archive.","error");
    if(file.size>250*1024*1024)return notify("Maximum ZIP size is 250 MB.","error");
    try{
    ...
    }catch(e){notify(e.message,"error");setStatus("")}finally{setBusy(false)}
  }
  return <main className="page narrow"><div className="eyebrow">SELLER</div><h1>Publish software</h1><p className="muted">Your source archive goes directly into private storage and must pass validation before publishing.</p>
    <form className="form" onSubmit={submit} noValidate>
      <label>Name<input name="name" value={f.name} onChange={set} className={fieldErrors.name?"invalid":""}/>{fieldErrors.name&&<span className="fieldError">{fieldErrors.name}</span>}</label>
      <label>Slug<input name="slug" value={f.slug} onChange={set} placeholder="my-saas-starter" className={fieldErrors.slug?"invalid":""}/>{fieldErrors.slug&&<span className="fieldError">{fieldErrors.slug}</span>}</label>
      <label>Description<textarea name="description" value={f.description} onChange={set} rows="5" className={fieldErrors.description?"invalid":""}/>{fieldErrors.description&&<span className="fieldError">{fieldErrors.description}</span>}</label>
      <div className="formGrid"><label>Category<Select name="category" value={f.category} onChange={v=>setF({...f,category:v})} options={["SaaS","React","Node.js","AI","APIs","UI Kits","Templates","Developer Tools"]}/></label><label>Price (NGN)<input name="price" type="number" min="100" value={f.price} onChange={set} className={fieldErrors.price?"invalid":""}/>{fieldErrors.price&&<span className="fieldError">{fieldErrors.price}</span>}</label></div>
      <label>Technologies<input name="technologies" value={f.technologies} onChange={set}/></label>
      <label>License<Select name="license" value={f.license} onChange={v=>setF({...f,license:v})} options={["MIT","Apache 2.0","GPL","Personal Use","Commercial Use","Extended Commercial","Custom License"]}/></label>
      <div className="formGrid"><label>Version<input name="version" value={f.version} onChange={set} className={fieldErrors.version?"invalid":""}/>{fieldErrors.version&&<span className="fieldError">{fieldErrors.version}</span>}</label><label>Demo URL<input name="demoUrl" value={f.demoUrl} onChange={set} placeholder="https://…"/></label></div>
      <label>Source ZIP<input type="file" accept=".zip,application/zip" onChange={e=>{setFile(e.target.files?.[0]||null); if(fieldErrors.file)setFieldErrors({...fieldErrors,file:null})}}/><small>ZIP only · 250 MB maximum</small>{fieldErrors.file&&<span className="fieldError">{fieldErrors.file}</span>}</label>
