...
// Adapts a row from /api/marketplace/products (real schema columns) into the
...
function NewProduct({user}){
  ...
  async function submit(e){
    ...
    try{
      ...
      const prep=await api("/api/uploads/source-url",{method:"POST",headers:h,body:JSON.stringify({productId:d.product.id,version:f.version,fileName:file.name,fileSize:file.size,changelog:f.changelog})});
      ...
      const scan=await api("/api/uploads/finalize",{method:"POST",headers:h,body:JSON.stringify({productId:d.product.id,fileId:prep.fileId,path:prep.path,fileSize:file.size})});
      if(scan.scan?.status!=="clean")throw new Error("Upload quarantined by security validation.");
      setStatus("Publishing…");
      await api("/api/products/publish",{method:"POST",headers:h,body:JSON.stringify({productId:d.product.id})});
...
function Downloads(){
  ...
  async function download(){try{const d=await api("/api/downloads/order?orderId="+encodeURIComponent(orderId));window.location.href=d.url}catch(e){setMsg(e.message)}}
...
function Notifications(){
  ...
  async function read(id){await api("/api/notifications/read",{method:"POST",body:JSON.stringify({id})});load()}
...
function VerifyLicense(){const {key}=useParams();const [d,setD]=React.useState(null);React.useEffect(()=>{fetch("/api/licenses/verify?key="+encodeURIComponent(key)).then(r=>r.json()).then(setD)},[key]);return <main className="authPage"><div className="authCard"><div className="eyebrow">LICENSE VERIFICATION</div><h1>{d?.valid?"Valid license":"License verification"}</h1><code>{key}</code><p>{d?.message||"Checking license…"}</p></div></main>}
