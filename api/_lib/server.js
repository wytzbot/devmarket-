...
// Built lazily (not at module load) so a missing env var turns into a clean,
// catchable JSON error response instead of crashing the whole function
// before any request handler even runs.
let _db = null;
function getDb() {
  if (_db) return _db;
  if (!SUPABASE_URL || !SUPABASE_KEY) {
    ...
    e.statusCode = 500;
    e.detail = 'SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY is not set in this deployment\'s environment variables.';
    throw e;
  }
...
export function safeError(error) { console.error(error, error?.detail || ''); return error?.message && /^[A-Z0-9_]+$/.test(error.message) ? error.message : 'REQUEST_FAILED'; }
